...
class CountryData
  ...
  def self.highest_gni_per_capita_in_asia
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent = 'Asia' AND gni >= ALL(SELECT gni FROM countries WHERE continent = 'Asia' AND gni >0);")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries ORDER BY gni LIMIT 1 OFFSET 2;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
